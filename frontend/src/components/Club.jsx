  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-black via-gray-900 to-black text-white pt-20 px-4 relative">
      ...
      <div className="relative z-10 max-w-6xl mx-auto bg-black/60 p-6 sm:p-10 rounded-3xl shadow-xl border border-yellow-400">
        ...
        {/* Search */}
        <form onSubmit={handleSearch} className="flex flex-col sm:flex-row justify-center items-center mb-10 gap-4">
          <input
            type="text"
          ...
          <div className="flex gap-2">
            <button type="submit" className="bg-yellow-400 hover:bg-yellow-500 text-black font-semibold px-5 py-2 rounded-full shadow">
              Search
            </button>
            <button type="button" onClick={handleReset} className="bg-gray-600 hover:bg-gray-700 text-white px-5 py-2 rounded-full shadow">
              ↺ Reset
            </button>
          </div>
        </form>
          ...
          {selectedClub && (
            <div className="absolute top-4 right-4 bg-black/80 border border-yellow-400 rounded-xl p-5 w-full sm:w-96 z-[1000] shadow-lg">
              <div className="flex items-center mb-4">
                ...
                />
                <div>
                  <h2 className="text-xl font-bold text-yellow-300">{selectedClub.name}</h2>
              ...
              <p className="text-gray-300 text-sm mb-3">{selectedClub.description}</p>
              <div className="text-sm text-gray-300 mb-3">
                <strong className="text-yellow-300">Conducted Competitions:</strong>
        ...
        <div className="text-center mt-6">
          {!showAddClubForm ? (
            ...
            >
              ...
              <p className="text-sm text-gray-900">Click to share your club with others!</p>
            </div>
          ) : (
            <div className="bg-black/70 border border-yellow-400 rounded-2xl p-6 shadow-xl w-full sm:w-[32rem] mx-auto text-white">
              <h3 className="text-2xl font-bold mb-4 text-yellow-300">Add Your Club</h3>
              <form className="space-y-4" onSubmit={handleAddClubSubmit}>
                <input name="name" type="text" placeholder="Club Name" value={newClub.name} onChange={handleNewClubChange} className="w-full p-3 border rounded-md bg-gray-800 text-white border-yellow-400" required />
                <input name="email" type="email" placeholder="Email" value={newClub.email} onChange={handleNewClubChange} className="w-full p-3 border rounded-md bg-gray-800 text-white border-yellow-400" required />
                <input name="phone" type="text" placeholder="Phone Number" value={newClub.phone} onChange={handleNewClubChange} className="w-full p-3 border rounded-md bg-gray-800 text-white border-yellow-400" />
                <input name="city" type="text" placeholder="City" value={newClub.city} onChange={handleNewClubChange} className="w-full p-3 border rounded-md bg-gray-800 text-white border-yellow-400" />
                <textarea name="description" placeholder="Description" value={newClub.description} onChange={handleNewClubChange} className="w-full p-3 border rounded-md bg-gray-800 text-white border-yellow-400" rows="3" />
                <input name="locationLink" type="text" placeholder="Google Maps Link" value={newClub.locationLink} onChange={handleNewClubChange} className="w-full p-3 border rounded-md bg-gray-800 text-white border-yellow-400" />
                <input name="competitions" type="text" placeholder="Competitions (comma separated)" value={newClub.competitions} onChange={handleNewClubChange} className="w-full p-3 border rounded-md bg-gray-800 text-white border-yellow-400" />
                <input type="file" name="img" accept="image/*" onChange={handleImageChange} required className="text-sm text-white" />
  
                <button type="submit" className="w-full bg-yellow-400 hover:bg-yellow-500 text-black font-semibold px-4 py-3 rounded-md">
                  Submit
                </button>
                <button type="button" onClick={handleCancel} className="w-full bg-gray-600 hover:bg-gray-700 text-white px-4 py-3 rounded-md">
                  Cancel
                </button>
              </form>
            </div>
